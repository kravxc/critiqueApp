<template>
  <div class="home">
    <section class="hero" :class="{ 'hero--loaded': heroLoaded }">
      ...
      <button class="hero__scroll" @click="scrollToReviews" aria-label="Прокрутить вниз">
        <span></span>
      </button>
    </section>
    ...
    <section id="reviews" class="reviews" ref="reviewsSection">
      <div class="container">
        <div class="section-header" :class="{ 'section-header--visible': reviewsVisible }">
        ...
        <div v-if="isLoading" class="reviews__grid">
          <div v-for="i in 6" :key="i" class="review-card review-card--skeleton">
        ...
        <div v-else-if="reviews.length > 0" class="reviews__grid">
          <TransitionGroup name="card" @before-enter="beforeCardEnter" @enter="cardEnter">
            <article
            ...
            >
              <div class="review-card__content">
                <div class="review-card__meta">
                  <span class="review-card__author-name">{{ review.user?.name || 'Пользователь' }}</span>
                  <span class="review-card__date">{{ formatDate(review.created_at) }}</span>
                </div>

                <h3 class="review-card__title">{{ review.title }}</h3>
                <p class="review-card__excerpt">{{ truncateText(review.text, 150) }}</p>
        ...
        <div v-else-if="error" class="reviews__error">
          ...
          <p>{{ error }}</p>
          <button @click="fetchPopularReviews" class="btn btn--outline">Попробовать снова</button>
        </div>
    ...
    <section id="features" class="features" ref="featuresSection">
      <div class="container">
        <div class="section-header" :class="{ 'section-header--visible': featuresVisible }">
          <h2 class="section-title">Почему выбирают Наше мнение?</h2>
          <p class="section-subtitle">Мы создаем пространство для истинных ценителей звука</p>
        </div>
    ...
    <section class="cta" ref="ctaSection" :class="{ 'cta--visible': ctaVisible }">
      <div class="container">
...
const features = ref([
  {
    ...
    description: 'Никакой цензуры и влияния лейблов. Только честные эмоции и профессиональный анализ.'
  },
  {
    id: 2,
    icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>`,
    title: 'Активное сообщество',
    description: 'Обсуждайте альбомы, спорьте о треках и находите единомышленников в комментариях.'
  },
  {
    id: 3,
    ...
    description: 'Наши авторы — музыканты, критики и журналисты с многолетним стажем.'
  }
])
</script>

<style lang="scss" src="@/assets/styles/home.scss" scoped>
